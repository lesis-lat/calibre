    ...
    sub new {
        ...
        for my $account (@active_accounts) {
            ...
            for my $query_name ( keys %{$queries} ) {
                ...
                $output = _run_command( $command, $account_name, $query_name );
        ...
        if ( $report_type eq 'multiple' ) {
            for my $account_name ( keys %account_reports ) {
                ...
                for my $query_name ( keys %{ $account_reports{$account_name} } )
                {
                    ...
                    my $description =
                      $account_reports{$account_name}->{$query_name}
                      ->{description};
                    my $output =
                      $account_reports{$account_name}->{$query_name}->{output};
                    $output =~ s/^/      /mgxs;
                    ...
                    _write_report_file( $output_file, $content );
                }
            }
        ...
        for my $account_name ( keys %account_reports ) {
            ...
            for my $query_name ( keys %{ $account_reports{$account_name} } ) {
                my $description =
                  $account_reports{$account_name}->{$query_name}->{description};
                my $output =
                  $account_reports{$account_name}->{$query_name}->{output};
        ...
        if ($has_query_groups) {
            ...
            for my $category ( sort keys %{$query_groups} ) {
                ...
                if ( ref $files ne 'ARRAY' ) {
                    croak
"Query category '$category' must be a list in '$input_file'.";
                }

                for my $file ( @{$files} ) {
                    my $resolved_file =
                      _resolve_query_file( $input_file, $file );
                    my $file_queries = LoadFile($resolved_file);

                    if ( ref $file_queries ne 'HASH' ) {
                        croak
"Query file '$resolved_file' must contain a YAML map.";
                    }

                    for my $query_name ( keys %{$file_queries} ) {
                        if ( exists $merged_queries{$query_name} ) {
                            croak
"Duplicate query name '$query_name' found in '$resolved_file'.";
                        }

                        $merged_queries{$query_name} =
                          $file_queries->{$query_name};
                    }
                }
            }
    ...
    sub _run_command {
        my ( $command, $account_name, $query_name ) = @_;
        my $command_handle;
        if ( !open $command_handle, q{-|}, $command ) {
            my $command_error = $ERRNO;
            croak
              "Failed to run steampipe query for $account_name: $command_error";
        }

        local $INPUT_RECORD_SEPARATOR = undef;
        my $output = <$command_handle>;

        if ( !close $command_handle ) {
            my $command_error = $ERRNO;
            carp
"Error running query '$query_name' for $account_name: $command_error";
        }
    ...
    sub _resolve_query_file {
        ...
        croak
          "Query file '$query_file' referenced in '$input_file' was not found.";
    }
}
